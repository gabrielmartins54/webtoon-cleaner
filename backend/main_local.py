...
def process_images(
    ...
    for img_path in images:
        ...
        layer_bg = nested_layers.Image(
            name='Original', visible=True, top=0, left=0, bottom=orig_rgba.shape[0], right=orig_rgba.shape[1],
            channels={0: orig_rgba[:,:,0], 1: orig_rgba[:,:,1], 2: orig_rgba[:,:,2], -1: orig_rgba[:,:,3]}
        )
        
        # pytoshop expects bottom-to-top layer order
        # Check if we need PSB (Large Document Format) for very long webtoon strips
        from pytoshop.enums import Version
        psd_version = Version.psb if max(orig_rgba.shape[:2]) > 30000 else Version.version_1
        
        psd = nested_layers.nested_layers_to_psd(
            [layer_clean, layer_bg], 
...
def upload_to_gdrive(zip_path: Path):
    """ Uploads file to Google Drive. Needs Service Account credentials.json """
    creds_file = 'credentials.json'
    if not os.path.exists(creds_file):
        print("No credentials.json found. Skip GDrive upload.")
        return False
        
    try:
        # Check if JSON is Service Account or OAuth Client
        import json
        with open(creds_file) as f:
            data = json.load(f)
            if 'client_email' not in data:
                print("Error: credentials.json is NOT a Service Account key. (Looks like OAuth Client ID)")
                return False
        ...
        # Found in URL: drive.google.com/drive/folders/ID_HERE
        FOLDER_ID = '1qbiiz47QKsopYI-a5QxNJFf4sYwBm_bF' 
...
async def upload_file(
    ...
    if int(process_summary.get("processed", 0)) <= 0:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "Check CUDA/Torch runtime and LaMa model startup logs."
            ),
        )
    ...
    # Upload GDrive
    upload_success = upload_to_gdrive(result_zip_path)
    msg = "Cleaned, zipped, uploaded." if upload_success else "Cleaned, zipped. GDrive skipped (no creds)."
